...
def get_model_id_gdrive(model_type):
    ...
    # corrected 100d word_index / embeddings IDs (Discord discussion)
...
def get_category_embeddings(embeddings_metadata):
    ...
    for category in st.session_state.categories.split(" "):
        if embedding_model == "openai":
            ...
            if category not in st.session_state[cache_key]:
                st.session_state[cache_key][category] = (
                    get_sentence_transformer_embeddings(category, model_name=model_name)
                )
        else:
            if category not in st.session_state[cache_key]:
                st.session_state[cache_key][category] = (
                    get_sentence_transformer_embeddings(category)
                )
...
def cosine_similarity(x, y):
    ...
    (20 pts)
    """
    dot_product = np.dot(x, y)  # dot(x, y)

    norm_x = la.norm(x)  # ||x||
    norm_y = la.norm(y)  # ||y||

    if norm_x == 0 or norm_y == 0:
        # avoid division by zero when one of the vectors is all zeros
        raise ValueError("Cannot compute cosine similarity with a zero vector.")

    cosine_sim = dot_product / (norm_x * norm_y)  # cosine similarity
    return np.exp(cosine_sim)  # exponentiated cosine score


# Task II: Average Glove Embedding Calculation
def averaged_glove_embeddings_gdrive(
    sentence, word_index_dict, embeddings, model_type="50d"
):
    """
    ...
    embedding = np.zeros(embedding_dim)  # running sum of word embeddings

    tokens = sentence.split()  # simple whitespace tokenization

    valid_embeddings = 0  # count words that exist in GloVe vocab

    for token in tokens:
        if token.lower() in word_index_dict:
            embedding += get_glove_embeddings(
                token, word_index_dict, embeddings, model_type
            )  # add each valid word embedding
            valid_embeddings += 1

    if valid_embeddings == 0:
        # if no tokens found in vocab, return zero vector
        return embedding

    return embedding / valid_embeddings  # average over valid word embeddings
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # embed the input sentence using averaged GloVe embeddings
        ...
        for index, category in enumerate(categories):
            # embed each category label with the same GloVe averaging
            category_embedding = averaged_glove_embeddings_gdrive(
                category, word_index_dict, embeddings, model_type
            )

            try:
                # cosine similarity between sentence and this category
                cosine_sim[index] = cosine_similarity(
                    sentence_embedding, category_embedding
                )
            except ValueError:
                # fallback when one of the vectors is zero
                cosine_sim[index] = 0.0

    elif embeddings_metadata["embedding_model"] == "openai":
        # Extract OpenAI-specific parameters
        model_name = embeddings_metadata["model_name"]
        cache_key = "cat_embed_openai_" + model_name  # per-model category cache key

        # embed the input sentence with OpenAI embeddings
        sentence_embedding = get_openai_embeddings(
            st.session_state.text_search, model_name=model_name
        )

        # lazily initialize / refresh cached category embeddings
        if cache_key not in st.session_state:
            update_category_embeddings(embeddings_metadata)
        all_cats_populated = set(categories).issubset(
            st.session_state[cache_key].keys()
        )
        if not all_cats_populated:
            update_category_embeddings(embeddings_metadata)

        for index, category in enumerate(categories):
            try:
                # cosine similarity between sentence and cached category embedding
                cosine_sim[index] = cosine_similarity(
                    sentence_embedding, st.session_state[cache_key][category]
                )
            except ValueError:
                cosine_sim[index] = 0.0  # handle possible zero vectors

    else:  # Sentence transformers
        # Extract Sentence Transformer-specific parameters
        model_name = embeddings_metadata["model_name"]
        cache_key = "cat_embed_transformers_" + (
            model_name if model_name else "default"
        )  # per-model transformer cache key

        # embed the input sentence with the sentence transformer
    ...
    # sort categories by similarity score, highest first
    sorted_cosine_scores = sorted(
        list(cosine_sim.items()), key=lambda item: item[1], reverse=True
    )
...
if __name__ == "__main__":
    ...
    # set initial defaults so widgets remember previous values
    ...
    if ACCESS_CODE is None:
        st.warning(
            "ACCESS_CODE is not set. Please configure it in secrets or environment."
        )
    else:
        if not st.session_state.authenticated:
            st.title("🔒 Restricted Assignment App")
    ...
    model_type = st.sidebar.selectbox(
        "Choose the model", ("25d", "50d", "100d"), index=1
    )
    ...
    if st.session_state.text_search:
        ...
        with st.spinner("Obtaining Cosine similarity for OpenAI Small (1536d)..."):
            sorted_cosine_sim_openai_small = get_sorted_cosine_similarity(
                embeddings_metadata
            )
        ...
        with st.spinner("Obtaining Cosine similarity for OpenAI Large (3072d)..."):
            sorted_cosine_sim_openai_large = get_sorted_cosine_similarity(
                embeddings_metadata
            )
        ...
        st.write("")
        st.write("Demo developed by [Lorin Lee](https://www.linkedin.com/in/leelorin/)")
